function obj = calculate_objectiveDNase ( modmean, tf_expression, assign, nonEmptyModules, W, DNaseRel, cellTypeMap, DNasePairWeights, d, e )
...
%       W: a matrix of size num_mods x r, that contains the coefficients 
%           for each module, regulator pair
...
for ct = 1:length(cellTypeMap)
    ...
    % objective
    objectiveWithMetapriorVarsCellType = objectiveWithMetapriorVarsCellType + weightTimesDNaseRelAverage(:,:,cellTypeMap(ct)) - permute(log(DNaseRelAverage(:,nonEmptyModules,cellTypeMap(ct))), [2 1 3]) + (d*(W(nonEmptyModules,:).^2));
end

obj = sum(mainObj + sum(objectiveWithMetapriorVarsCellType, 2)) + (e*sum(sum(DNasePairWeights.^2, 2)));
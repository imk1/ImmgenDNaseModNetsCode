function params = lirnetDNase(expression, tf_expression, assign, DNasePairFeats, cellTypeMap, DNaseRel, num_mod, maxiters, reclust, l1Bounds, d, e)
...
DNasePairWeights = rand(length(DNasePairFeats), nregs);
...
for rr = 1:reclust;
    ...
    W = learn_weightsDNase (tf_expression, modmean, assign, DNaseRel, cellTypeMap, d); % Allowing only regulators to be features
    if rr == 1
        % Store the initial weights with no learned meta-priors
        params.W0 = W;
    end
    
    obj = calculate_objectiveDNase ( modmean, tf_expression, assign, nonEmptyModules, W, DNaseRel, cellTypeMap, DNasePairWeights, d, e );

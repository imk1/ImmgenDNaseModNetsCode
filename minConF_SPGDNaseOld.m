function [DNasePairWeights,f,funEvals,projects] = minConF_SPGDNase(funObj,DNasePairWeights,funProj, options)
...
while funEvals <= maxIter
    ...
    if ~curvilinear
        d = funProj(DNasePairWeights+d)-DNasePairWeights;
        projects = projects+1;
    end
    ...
    if curvilinear
        x_new = funProj(DNasePairWeights + t*d);
        projects = projects+1;
    else
    ...
    lineSearchIters = 1;
    while f_new > funRef + suffDec*g'*(x_new-DNasePairWeights) || ~isLegal(f_new)
        temp = t;

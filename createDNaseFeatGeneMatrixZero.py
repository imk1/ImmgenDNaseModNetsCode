def createDNaseFeatGeneMatrixZero(genesToDNaseFileName, genesDNaseFeatFileName, numDNaseSites, numGenes):
	...
	# 0 for non-binary features means that there is not feature for the gene, either because the DNase site is not near the gene's TSS or because the information for that feature was not available
	# ASSUMES THAT, FOR EACH BINARY FEATURE, IF THE BINARY FEATURE IS NOT PRESENT, SOME OTHER FEATURE WILL NOT BE PRESENT
	...
	for i in range(numDNaseSites):
		...
		for j in range(numGenes):
			# Initialize DNaseFeatGeneMat all of the features to 0
			DNaseFeatGeneMatRow.append(0)
	...
	for line in genesToDNaseFile:
		...
		for DNaseIndexStr in lineElements[4:len(lineElements)]:
			...
			if featVal != -1:
				# There is information for the feature, so put it into the matrix
...
if __name__=="__main__":
    ...
    DNaseFeatGeneMat = createDNaseFeatGeneMatrixZero(genesToDNaseFileName, genesDNaseFeatFileName, numDNaseSites, numGenes)

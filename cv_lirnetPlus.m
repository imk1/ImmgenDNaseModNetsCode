function [error,rsquare,rsquareModule,weights,assign] = cv_lirnetPlus(X,Y,l1,l2,num_mod,nfolds)
...
for ii = 1:nfolds
    ...
    for jj = 1:length(l1)
        ...
        params = lirnet_flat_fullPlus(Xtrn',Ytrn',num_mod,l1(jj),l2(jj)); % X is all expressions, Y is TF expressions
        ...
        for k = 1:num_mod
            ...
            % module
            modLocations = find(params.assign(:,end) == k);
            if length(modLocations) > 0
                % There is at least 1 gene in the module

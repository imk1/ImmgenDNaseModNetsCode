...
[f,g] = funObj(DNasePairWeights, DNasePairFeats, cellTypeMap, W, assign, nonEmptyModules, l1Upper, l1Lower, e);
funEvals = 1;

% Check Optimality of Initial Point
projects = projects+1;
if sum(abs(funProj(DNasePairWeights-g)-DNasePairWeights)) < optTol
    if verbose >= 1
        fprintf('First-Order Optimality Conditions Below optTol at Initial Point, Gradient entry is %g\n', g(1));
    end
    return;
end
...
while funEvals <= maxIter
    ...
    if i == 1
        ...
        if bbInit
            ...
            % Solve Sub-problem
            xSubInit = DNasePairWeights-alpha*g;
            feasibleInit = 0;
        else
            xSubInit = DNasePairWeights;
            feasibleInit = 1;
        end
        % Solve Sub-problem
    ...
    x_new = DNasePairWeights + t*d;
    [f_new,g_new] = funObj(x_new, DNasePairFeats, cellTypeMap, W, assign, nonEmptyModules, l1Upper, l1Lower, e);
    funEvals = funEvals+1;
    ...
    optCond = sum(abs(funProj(DNasePairWeights-g)-DNasePairWeights));
    projects = projects+1;
    ...
    % Check optimality
        if optCond < optTol
            fprintf('First-Order Optimality Conditions Below optTol, Gradient entry is %g\n', g(1));
            break;
        end
...
[p,f,funEvals,subProjects] = minConF_SPGDNase(funObj,x_init,funProj, options);
end

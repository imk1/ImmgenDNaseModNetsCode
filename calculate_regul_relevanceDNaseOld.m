...
DNaseRel = zeros( size( DNasePairFeats{1},2), ngenes, nCellTypes ); % Number of regulators by number of genes by number of cell types with DNase

for m = 1 : ngenes
   % Iterate through genes and get their DNase regulator relevances
   ...
   DNaseRel(:,m,:) = l1Upper * (ones(size(rel)) - computeSigmoidMat( rel )) + l1Lower;
end